...
function validateFeaturedSeries(value: unknown, errors: string[]) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    if (!isNonEmptyString(series.slug)) errors.push(`featuredSeries[${index}].slug 不能为空。`);
    if (!isNonEmptyString(series.slug)) continue;
...
export function validateSiteConfigContent(content: string): string[] {
  ...
  if (!site) {
    ...
    if (site.keywords !== undefined && !Array.isArray(site.keywords)) {
      errors.push('site.keywords 必须是数组。');
    }
  }
